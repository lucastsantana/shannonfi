import { TradeRecord } from '../mb/types';
...
export class PnlService {
  ...
  async logRebalance(record: TradeRecord): Promise<void> {
    ...
    logger.info('=== Rebalance Summary ===', {
      ...
      fillPrice: record.fillPrice?.toFixed(2) ?? 'N/A',
      feeBrl: record.feeBrl?.toFixed(2) ?? 'N/A',
      portfolioValueBefore: before.totalValueBrl.toFixed(2),
      portfolioValueAfter: after?.totalValueBrl.toFixed(2) ?? 'N/A',
