import { PortfolioSnapshot, TradeRecord } from '../mb/types';
...
export class MetricsService {
  ...
  computeMetrics(snapshots: PortfolioSnapshot[]): TrackRecordMetrics {
    ...
    const filled = trades.filter(
      (t): t is TradeRecord => t.status === 'filled' || t.status === 'DRY_RUN',
    );
    ...
    const totalReturnBrlPct =
      initialBrl > 0 ? ((finalBrl - initialBrl) / initialBrl) * 100 : 0;
    ...
    const maxDrawdownPct = this.computeMaxDrawdown(sorted.map((s) => s.totalValueBrl));
    const sharpeRatio = this.computeSharpe(sorted.map((s) => s.totalValueBrl));
  ...
  printReport(snapshots: PortfolioSnapshot[]): void {
    ...
    console.log("\n=== Shannon's Demon MB — Track Record ===");
    ...
    if (m.cagr != null) {
      console.log(`CAGR (BRL):           ${sign(m.cagr)}${m.cagr.toFixed(2)}%`);
    }
