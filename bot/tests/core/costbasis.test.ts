...
describe('CostBasisService (BRL-native AVCO)', () => {
  ...
  it('starts with empty ledger', () => {
    ...
    expect(ledger.base.averageCostBrl).toBe(0);
    expect(ledger.base.totalBase).toBe(0);
  });

  it('updateAfterBuy sets correct average cost', () => {
    svc.updateAfterBuy(2, 800);
    const ledger = svc.getLedger();
    expect(ledger.base.averageCostBrl).toBeCloseTo(400, 5);
    expect(ledger.base.totalBase).toBeCloseTo(2, 5);
  });
  ...
  it('updateAfterBuy computes weighted average on second buy', () => {
    ...
    const ledger = svc.getLedger();
    expect(ledger.base.averageCostBrl).toBeCloseTo(450, 5);
    expect(ledger.base.totalBase).toBeCloseTo(4, 5);
  });
  ...
  it('computeRealizedGainBrl is pure (no side effects)', () => {
    ...
    expect(svc.getLedger().base.totalBase).toBeCloseTo(2, 5); // unchanged
  });
